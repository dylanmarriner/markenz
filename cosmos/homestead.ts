...
import { RedRoom } from './red-room';

export interface Building {
  id: string;
  type: 'shelter' | 'workshop' | 'storage' | 'garden' | 'well' | 'fireplace' | 'armory' | 'red_room';
...
export class Homestead {
  ...
  update(dtSeconds: number, weather: { temperature: number; precipitation: number; wind: number }): void {
    ...
    // Update armory
    this.armory.update(dtSeconds);

    // Update red room
    this.redRoom.update(dtSeconds);
  }
  ...
  private _initializeHomestead(): void {
    ...
    this.addBuilding({
      type: 'armory',
      position: { x: 20, y: 0 },
      size: { width: 15, height: 4, depth: 8 },
      resources: new Map(),
      condition: 1.0,
      functionality: 1.0
    });
  ...
  getHomesteadReport(): {
    ...
    return {
      ...
      armory: this.armory.getArmoryReport(),
      red_room: this.redRoom.getRedRoomReport()
    };
  }
